...
class AutomatedReasoner:
    ...
    def forward_chain(self, max_iterations: int = 10) -> List[Fact]:
        ...
        while new_facts and it < max_iterations:
            ...
            for rule in self.kb.rules:
                subjects = set(f.subject for f in self.kb.facts.values())
                for subject in subjects:
                    ok = all(self.kb.has_fact(p, subject, True) or
                             (p.startswith("NOT_") and not self.kb.has_fact(p.replace("NOT_",""), subject, True))
                             for p in rule.premises if not p.startswith("NOT_") or True)
                    ...
                    if premises_ok:
                        ...
                        if key not in self.kb.facts:
                            ...
                            self.proof_steps.append(f"Step: {rule.premises} for {subject} → {nf} via {rule.name}")
                            new_facts = True
    ...
    def prove_no_evidence_theorem(self, claim_id: str) -> Dict:
        ...
        if not has_ev:
            ...
            self.proof_steps.append(f"Theorem Proved: ¬HasEvidence({claim_id}) → ¬Verifiable({claim_id}) | QED")
            return {"proved": True, "verdict": "REJECTED - Mathematically Proven", "proof": self.proof_steps}
