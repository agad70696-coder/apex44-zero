...
class Qubit:
    ...
    def is_zero(self):
        return abs(self.beta) < 0.01

    def is_one(self):
        return abs(self.alpha) < 0.01

    def is_superposition(self) -> bool:
        return not self.is_zero() and not self.is_one()
...
class QuantumState:
    ...
    def put_superposition(self) -> None:
        for q in self.qubits:
            q.alpha = complex(1 / math.sqrt(2), 0)
            q.beta = complex(1 / math.sqrt(2), 0)
...
class CNOT:
    ...
    def apply(control: Qubit, target: Qubit):
        if control.is_one() or (
            control.is_superposition() and random.random() < abs(control.beta) ** 2
        ):
