...
class NanoMaterialEvidence:
    ...
    """
    def __init__(self, material_id: str, material_type: str = "graphene", conductivity: float = 100.0):
        self.material_id = material_id
        self.material_type = material_type  # graphene, nanotube, smart_polymer
        self.conductivity = conductivity
        self.timestamp = str(time.time())
        self.base_hash = material_hash(f"{material_id}{material_type}{conductivity}{self.timestamp}")
        self.logs = []

    def apply_stress(self, stress_level: float, temperature: float):
        # المواد الذكية بتتأثر بالحرارة والضغط
        old = self.conductivity
        self.conductivity = self.conductivity * (1 - stress_level*0.01) * (1 - (temperature-25)*0.002)
        entry = {
            "stress": stress_level,
            "temp": temperature,
