import sys; import pathlib; sys.path.insert(0, str(pathlib.Path(__file__).resolve().parents[2]))
...
class OpportunityScanner:
 ...
 def scan_tla(self):
  ...
  if not p.exists(): return {"status":"FAIL","opportunity":"TLA missing"}
  txt=p.read_text()
 ...
 def run(self):
  r={"rpc":self.scan_rpc(),"jcs":self.scan_jcs(),"tla":self.scan_tla(),"nist":self.scan_nist()}
  print(r)
  return r
