    ...
    def verify(self, merkle_root: str, tx_hash: str | None = None) -> dict[str, Any]:
        ...
        try:
            if not self.w3:
                return {
                    "verified": False,
                    "reason": "w3 provider not configured",
                    "tx_hash": tx_hash,
                }
            ...
            expected = (
                self.w3.keccak(text=merkle_root).hex()
                if hasattr(self.w3, "keccak")
                else "0x" + hashlib.sha3_256(merkle_root.encode()).hexdigest()
            )
            tx_input = (
                getattr(tx, "input", None) or tx.get("input", "") if isinstance(tx, dict) else ""
            )
            ...
            return {
                ...
                and confirmations >= self.min_confirmations,
                "exists": True,
