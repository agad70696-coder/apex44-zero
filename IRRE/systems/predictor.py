...
class SystemPredictor:
    def __init__(self, system: ComplexSystem = None) -> None:
    ...
    def what_if_remove_component(self, cid: str) -> dict[str, Any]:
        ...
        return {
            "scenario": f"Remove {cid}",
            "original_health": orig_h,
            "predicted_health": new_h,
            "drop": orig_h - new_h,
            "cascade_size": len(affected),
            "affected": affected,
            "prediction": f"Removing {cid} drops health by {orig_h - new_h:.2f} and affects {len(affected)} - Butterfly Effect!",
        }
    ...
    def what_if_add_bias(self, rate: float = 0.8):
        ...
        return {
            "scenario": f"Bias {rate * 100:.0f}%",
            "system_health_after": sys_h,
            "prediction": f"{rate * 100:.0f}% bias → System {sys_h:.2f}",
        }
