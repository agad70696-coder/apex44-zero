...
class EgyptianCulturalContext:
    ...
    def _build(self):
        self.expressions["ابنى"] = CulturalExpression("ابنى", "my son", "أمر تنفيذي سريع بثقة - ابن الطبقة كاملة حالاً", SocialIntent.BUILD_REQUEST, "BuildAction(layers=ALL, urgency=HIGH, lang=EG)")
        self.expressions["فين 3"] = CulturalExpression("فين 3", "where is 3", "فين الملف التالت؟ بنينا 2 وناقص التالت", SocialIntent.LOCATION_QUERY, "Query(file=3, context=prev2)")
        self.expressions["القاهرة مش كلمة"] = CulturalExpression("القاهرة مش كلمة", "Cairo not word", "القاهرة كيان جغرافي له علاقات داخل مصر، ليست String", SocialIntent.PHILOSOPHICAL_EXAMPLE, "Entity(Cairo,City)!= String('القاهرة')")
