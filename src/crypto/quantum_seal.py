...
DEV21 = ['114','124','126','135','156','157','207','224','234','241','245','330','337','342','352','360','386','397','419','424','428']
ZERO4 = ['260','322','336','384']
ALL25 = sorted(DEV21 + ZERO4, key=lambda x: int(x))
...
def seal_qid(qid: str, irre_hash: str) -> str:
    payload = f"APEX44|v2.4|QID:{qid}|IRRE:{irre_hash}|NIST:SHAKE-256-512|Grover:256bit|50Y".encode()
    return shake512_hex(payload)

def seal_all(qids, irre_hash):
    return {str(q): seal_qid(str(q), irre_hash) for q in sorted(qids, key=lambda x: int(x))}
...
if __name__ == "__main__":
    ...
    print(f"✅ Manifest saved: APEX44/04-data/quantum_seals_v2.4.json")
    print(f"Dev 25 = 21 answered (156 pairs) + 4 zero [260,322,336,384] - Baseline 0.0904/0.2260")
