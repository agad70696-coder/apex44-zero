...
def check_tayyib(halal, toxins):
    if halal and toxins: return "REJECT [2:168] ليس طيبا"
    if not halal: return "REJECT [2:168]"
    return "APPROVE [2:168]"

def check_khabaith(items):
    bad=["khinzir","maita","dam","khamr"]
    for i in items:
        if i in bad: return f"REJECT [7:157] خبيث {i}"
    return "APPROVE [7:157]"

def check_khamr(alcohol):
    if alcohol>0.5: return "REJECT [5:90] خمر"
    return "APPROVE [5:90]"

def check_haram(items):
    haram=["maita","dam","khinzir"]
    for i in items:
        if i in haram: return f"REJECT [2:173] حرام {i}"
