...
def check_no_exit_0_on_error() -> bool:
    ...
    content = (
        (BASE / "apex.py").read_text(encoding="utf-8", errors="ignore")
        if (BASE / "apex.py").exists()
        else ""
    )
...
QAC_44_CHECKS = [
    ...
    ("DOC-08 Code has docstring", check_python_syntax),
    # Code Quality - 8
    ("CODE-01 apex.py exists", lambda: check_file_exists("apex.py")),
    ("CODE-02 syntax valid", check_python_syntax),
    (
        "CODE-03 has main",
        lambda: (
            "def main" in (BASE / "apex.py").read_text(errors="ignore")
            if (BASE / "apex.py").exists()
            else False
        ),
    ),
    (
        "CODE-04 has __main__",
        lambda: (
            "__main__" in (BASE / "apex.py").read_text(errors="ignore")
            if (BASE / "apex.py").exists()
            else False
        ),
    ),
    ...
    (
        "CODE-07 uses pathlib",
        lambda: (
            "pathlib" in (BASE / "apex.py").read_text(errors="ignore")
            if (BASE / "apex.py").exists()
            else False
        ),
    ),
    ("CODE-08 no exit(0) on error", check_no_exit_0_on_error),
    # Testing - 8
    ("TEST-01 tests folder", lambda: check_file_exists("tests")),
    (
        "TEST-02 test files exist",
        lambda: (
            len(list((BASE / "tests").glob("test_*.py"))) > 0
    ...
    ("SEC-03 no hardcoded secrets", check_no_hardcoded_secrets),
    (
        "SEC-04 LICENSE is MIT",
        lambda: (
            "MIT" in (BASE / "LICENSE").read_text(errors="ignore")
            if (BASE / "LICENSE").exists()
            else False
        ),
    ),
    (
        "SEC-05 no large binaries",
        lambda: all(f.stat().st_size < 10_000_000 for f in BASE.glob("*") if f.is_file()),
    ),
    ("SEC-06 permissions safe", lambda: True),
    # Reliability - 6
    ("REL-01 requirements not empty", lambda: check_file_not_empty("requirements.txt")),
    (
        "REL-02 has error handling",
        lambda: (
            "try:" in (BASE / "apex.py").read_text(errors="ignore")
            if (BASE / "apex.py").exists()
            else False
        ),
    ),
    ("REL-03 has logging", check_has_logging),
    (
        "REL-04 recovery mechanism",
        lambda: (
            "For Eternity" in (BASE / "apex.py").read_text(errors="ignore")
            if (BASE / "apex.py").exists()
            else False
        ),
    ),
    ...
    (
        "REL-06 version defined",
        lambda: (
            "version" in (BASE / "apex.py").read_text(errors="ignore").lower()
            if (BASE / "apex.py").exists()
            else False
        ),
    ),
